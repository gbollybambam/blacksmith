...
## The Core Feature
Blacksmith is a centralized backend JSON API that accepts image file uploads (strictly JPEG, capped at 5MB to prevent memory exhaustion) and extracts the EXIF metadata. It returns that data as a JSON response. If a valid JPEG contains no EXIF data, the API safely returns an empty JSON object `{}` with a 200 OK status. After processing, it immediately discards the file.
